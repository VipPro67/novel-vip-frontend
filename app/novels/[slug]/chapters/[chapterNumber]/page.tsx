...
export default function ChapterPage() {
  ...
  useEffect(() => {
    ...
    const handleStorage = (event: StorageEvent) => {
      ...
      try {
        ...
        console.error(
          "Failed to parse updated reader settings from storage event",
          error
        );
      }
    };
  ...
  const chapterTypographyStyle = useMemo<
  ...
  useEffect(() => {
    ...
    audioUrl,
    effectiveReaderSettings?.audioAutoNextChapter,
    effectiveReaderSettings?.audioEnabled,
  ]);
  ...
  const fetchChapterContent = async (chapterData: ChapterDetail) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch chapter content: ${response.statusText}`
        );
      }
      ...
      return (
        <div className="flex items-center space-x-3 rounded-lg border border-muted p-4">
          ...
          <span className="text-sm text-muted-foreground">
            Generating audio it can take 3-5 minutes, you will get notification
            while it ready
          </span>
        </div>
      );
