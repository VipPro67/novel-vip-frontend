...
export class NovelManagementComponent implements OnInit {
  ...
  createNovel(novelData: NovelCreateDTO): void {
    this.adminService.createNovel(novelData).subscribe({
      next: (response) => {
        if (response.success) {
          this.snackBar.open("Novel created successfully", "Close", {
            duration: 3000,
          });
          this.loadNovels();
        } else {
  ...
  updateNovel(id: string, novelData: NovelUpdateDTO): void {
    this.adminService.updateNovel(id, novelData).subscribe({
      next: (response) => {
        if (response.success) {
          this.snackBar.open("Novel updated successfully", "Close", {
            duration: 3000,
          });
          this.loadNovels();
        } else {
  ...
  deleteNovel(id: string): void {
    if (confirm("Are you sure you want to delete this novel?")) {
      this.adminService.deleteNovel(id).subscribe({
        ...
        error: (error) => {
          this.snackBar.open(
            "Error deleting novel: " + error.message,
            "Close",
            {
              duration: 3000,
            }
  ...
  // Add methods for handling double-click and edit actions
  onRowDoubleClick(novel: NovelDTO): void {
